  ...
  app.get("/bots/:id", async (req, res) => {
    try {
      ...
      const [profileCount] = await manager.db.pool.execute(
        "SELECT COUNT(*) as count FROM profiles WHERE client_id = ?", [clientId]"use strict";
...
function startPanel({ manager, port, host }) {
  ...
  app.get("/bots/:id/settings", async (req, res) => {
    try {
      const clientId = req.params.id;
      const client = await manager.db.getClient(clientId);
      if (!client) return res.redirect("/bots");

      // Veritabanı metodları kontrolü
      const botSettings = manager.db.getBotSettings ? await manager.db.getBotSettings(clientId) : {};
      
      // Karakterleri al
      let characters = [];
      if (manager.db.getCharacters) {
        characters = await manager.db.getCharacters();
      } else {
        // Fallback: settings tablosundan veya default
        try {
          const charJson = await manager.db.getSetting("characters_json");
          characters = charJson ? JSON.parse(charJson) : [];
        } catch(e) {}
        if (!characters.length && manager.getDefaultCharacters) {
          characters = manager.getDefaultCharacters();
        }
      }

      const keywords = manager.db.getKeywords ? await manager.db.getKeywords(clientId) : [];
      const triggers = manager.db.getTriggers ? await manager.db.getTriggers(clientId) : [];

      res.render("bot-settings", {
        title: `Bot Ayarları: ${client.name || client.id}`,
        page: "bots",
        client: {
          ...client,
  ...
  app.post("/bots/:id/settings", async (req, res) => {
    try {
      if (manager.db.saveBotSettings) {
        await manager.db.saveBotSettings(req.params.id, req.body);
      }
      res.redirect(`/bots/${req.params.id}/settings?saved=true`);
    } catch (err) {
      console.error("Bot ayar kaydetme hatası:", err);
      res.redirect(`/bots/${req.params.id}/settings?error=true`);
    }
  });
  ...
  // Anahtar Kelimeler Sayfası (CONFLICT ÇÖZÜMÜ: BURASI EKLENDİ)
  ...
  // Humanization - Artık bot bazlı, yönlendir (CONFLICT ÇÖZÜMÜ: BURASI SEÇİLDİ)
...
// --- MERGE FIX: BOT AYARLARI SAYFASI ---
  app.get("/bots/:id/settings", async (req, res) => {
    try {
      const clientId = req.params.id;
      const client = await manager.db.getClient(clientId);
      ...
      // Veritabanı metodları kontrolü (Merge sırasında db.js güncellenmiş olmalı)
      const botSettings = manager.db.getBotSettings ? await manager.db.getBotSettings(clientId) : {};
